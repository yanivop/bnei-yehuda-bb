...
function renderFreeDates() {
  ...
  badge.textContent = free.length;
  badge.classList.toggle('none', free.length === 0);
  ...
  for (const d of free) {
    ...
    if (!byMonth.has(key)) byMonth.set(key, { label: \`\${monthNames[d.getMonth()]} \${d.getFullYear()}\`, days: [] });
    byMonth.get(key).days.push(d);
  }

  container.innerHTML = [...byMonth.values()].map(({ label, days }) => \`
